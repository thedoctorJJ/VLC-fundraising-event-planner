      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-1">
          {difficultyStars.map(star => (
            ...
            >
              <Star
                className={`w-5 h-5 transition-colors duration-150 ${star <= difficulty
                ...
                <tbody className="bg-white divide-y divide-slate-100">
                  {allEventMetrics.map((event) => (
                    <tr
                      key={event.id}
                      onClick={() => setSelectedId(event.id)}
                      className={`cursor-pointer transition-colors duration-150 ${event.id === selectedId
                        ? "bg-indigo-50/70 hover:bg-indigo-100" // Highlight selected row
                        : "hover:bg-slate-50"
                        }`}
                    >
                      <td className={`px-4 py-3 whitespace-nowrap text-sm font-medium ${event.id === selectedId ? "text-indigo-800 font-bold" : "text-slate-900"
                        }`}>
                        {event.name}
                      </td>
